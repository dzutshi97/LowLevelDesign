...
public class ClosestPalindrome {
    ...
    private long halfToPalindrome(long left, boolean even) {
        ...
        long res = left;
        if (!even) left = left / 10; //Discard last element in odd no case. Reason: in case of 123 as first half of 12345, reverse & append only 12 to the mirror image. 12321
        while (left > 0) {
            res = res * 10 + (left % 10); //include the last no
            left /= 10; //skip last no
        }
        return res;
    }
